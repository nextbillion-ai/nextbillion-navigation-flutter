...
mixin EventSimulator {
  ...
  static Future<void> inputText(String text) async =>
      Process.run('adb', ['shell', 'input', 'text', '$text']);

  //adb shell input tap 1290 960
  static Future<void> tapScreen(int x, int y) async =>
      Process.run('adb', ['shell', 'input', 'tab', '$x', '$y']);

  //adb shell input swipe 540 1600 540 100 1500
  static Future<void> swipe(
          int startX, int startY, int endX, int endY, int duration) async =>
      Process.run('adb', [
        'shell',
        'input',
        'swipe',
  ...
  //back button
  static Future<void> tapBackButton() async =>
      Process.run('adb', ['shell', 'input', 'keyevent', '4']);

  //home button
  static Future<void> tapHomeButton() async =>
      Process.run('adb', ['shell', 'input', 'keyevent', 'KEYCODE_HOME']);
